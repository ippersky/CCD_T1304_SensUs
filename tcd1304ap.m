...
s1 = serial("COM11", "BaudRate", 115200, "DataBits", 8); %, "databits", 8);
...
ICG_Period = 200000; % [microseconds]
...
% command to the nucleo. 
% Modify SH and ICG period, send number of expositions to average. 8 bits.
%instrfind

%% 
A = fread(s1); % 2*CCDsize, "uint8"); % Acquire data from serial port
...
dlmwrite('test.csv',B,'delimiter',',','-append');

%%
% Curve fitting
% Analysis
% result B - easier than with python/upython
